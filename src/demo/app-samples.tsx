...
import { TTimelineAPI } from '../models/TimelineModel';
import {
    ComponentContainer,
...
export const VerticalBasicNavigation: FunctionComponent<{
    type: string;
    items: TimelineItemModel[];
}> = ({ type, items }) => {

    const apiRef = useRef<TTimelineAPI>();
    ...
    return (
        ...
        vertically.
            ...
            <ComponentContainerTree type={type}>
                <div>
                    <button onClick={() => apiRef.current.navigateTo(0)}>First</button>
                    <button onClick={() => apiRef.current.navigateTo(7)}>8th</button>
                    <button onClick={() => apiRef.current.navigateTo(items.length - 1)}>Last</button>
                </div>
                <Chrono
                    items={items}
                    mode="VERTICAL"
