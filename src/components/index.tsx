...
import { TimelineProps, TTimelineAPI } from '../models/TimelineModel';
import GlobalContextProvider from './GlobalContext';
import Timeline from './timeline/timeline';
const toReactArray = React.Children.toArray;

const Chrono = React.forwardRef<TTimelineAPI, Partial<TimelineProps>>(
    (props, ref) => {
        const {
        ...
        useEffect(
            () => {
                if (initialActiveTimelineItem > 0) {
                    navigateTo(initialActiveTimelineItem);
                }
            },
            []
        )

        if ((ref as any).hasOwnProperty('current')) {
            (ref as any).current = {
                navigateTo,
            }
        }
