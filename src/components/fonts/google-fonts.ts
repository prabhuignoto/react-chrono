...
export function loadGoogleFonts(config: GoogleFontsConfig): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    link.onload = () => resolve();
    link.onerror = () => reject(new Error(`Failed to load Google Font: ${config.fontFamily}`));
 ...
 */
export function generateFontCssVars(config: GoogleFontsConfig): Record<string, string> {
  const cssVars: Record<string, string> = {};
  
  // Set base font family
  cssVars['--timeline-font-family'] = `"${config.fontFamily}", system-ui, ui-sans-serif, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif`;
