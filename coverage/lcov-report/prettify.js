...
function k(Z) {
  ...
  for (var V = 0, U = Z.length; V < U; ++V) {
    ...
    if (ae.ignoreCase) {
      ...
      if (/[a-z]/i.test(ae.source.replace(
  ...
  function X(am) {
    ...
    af.sort(function(av, au) { return (av[0] - au[0]) || (au[1] - av[1]) });
    var ai = [];
    var ap = [ NaN, NaN ];
  ...
  function W(al) {
    var aj = al.source.match(new RegExp(
...
function b(ad) {
  function Y(af) { return document.getElementsByTagName(af) }
    ...
    for (; X < T.length && W.now() < ag; X++) {
      ...
      if (ai.indexOf("prettyprint") >= 0) {
        ...
        for (var ak = aj.parentNode; ak; ak = ak.parentNode) {
          if ((ak.tagName === "pre" || ak.tagName === "code" ||
               ak.tagName === "xmp") &&
